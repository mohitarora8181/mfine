...
const Section1 = () => {
    return (
        <div className='w-full p-10 max-sm:p-5 flex flex-col gap-5'>
            <div className='w-full max-sm:pt-20 flex overflow-x-auto scrollbar-none px-40 max-sm:px-5 justify-center max-sm:justify-start gap-20 max-sm:gap-8 select-none'>
                {
                    home.section1.images.map((e, index) => {
                        return <span key={"sec1_img" + index} className='flex cursor-pointer flex-col h-full w-36 max-sm:min-w-20 align-middle gap-2'>
                            ...
                            <p className='self-center text-sm max-sm:text-[12px] max-sm:whitespace-normal whitespace-nowrap'>{e.text}</p>
                        </span>
                    }) 
                }
            </div>
            <div className='w-full flex px-32 max-sm:px-0 p-5 select-none relative'>
                ...
                <div className='absolute max-sm:-top-56 max-sm:w-full max-sm:left-0 bottom-0 left-[35%] w-1/3'>
                    <img className='absolute bottom-4 max-sm:top-3 left-3' width="18" height="18" src="https://dg0qqklufr26k.cloudfront.net/wp-content/uploads/2023/11/search_icon.svg" alt="Searh Icon"></img>
